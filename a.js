//开始服务启动计时器
 ...
 var funWebSvr = function (req, res){  
    ...
    if (pathName.charAt(pathName.length-1)=="/"){
     ...
     //使用路径解析模块,组装实际文件路径
    var filePath = libPath.join(Library/se/Assessment1b/indexed.html);
 ...
 //创建一个http服务器
 var webSvr=libHttp.createServer();
 
 //指定服务器错误事件响应
 webSvr.on("request", function(request, response) { 
    var url = request.url;
    if(url = '/'){
        libFs.readFile('./index.html', function(error, data){
            if(error){
                response.end('error there');
                return;
            }
            response.end(data);
 ...
 //开始侦听8125端口
 webSvr.listen(8125,function(){ //向控制台输出服务启动的信息
	console.log('[WebSvr][Start] running at http://127.0.0.1:8125/'); 
